...
game 3 would have been impossible because at one point the Elf showed you 20 red cubes at once; similarly, game 4 would also have been impossible because the Elf showed you 15 blue cubes at once. If you add up the IDs of the games that would have been possible, you get 8.
...
async function setup() {
    ...
    const parseConfig = {
        ...
        parseFunction: (text) => {
            ...
            output = output.map((x) => {
                const split = x.split(':');
                let pulls = split[1].split(';');
                pulls = pulls.map((pull, index) => {
                    let cubeCount = pull.split(',');
                    cubeCount = cubeCount.map((counts) => {
                        let [count, color] = counts.trim().split(' ');
                        return { count, color };
                    });
                    let output = {};
                    cubeCount.forEach(({ count, color }) => {
                        output[color] = Number(count);
                    });
                    return output;
                });
                const id = Number(split[0].substring(5));
                const output = { id, possible: true, pulls, power: 1 };
                return output;
            });
            return output;
        }
...
function getMax(pulls) {
    const output = { red: -Infinity, green: -Infinity, blue: -Infinity };

    pulls.forEach(({ red = 0, green = 0, blue = 0 }) => {
        console.log('red, green, blue :>> ', red, green, blue);
        if (red > output.red) output.red = red;
        if (green > output.green) output.green = green;
        if (blue > output.blue) output.blue = blue;
    });
    return output;
}
...
function main(input) {
    ...
    const games = input.map((game, index) => {
        if (DEBUG) console.log('=========================================');
        console.log('game.id :>> ', game.id);
        game.max = getMax(game.pulls);
        for (const [key, value] of Object.entries(game.max)) {
            console.log('key, value :>> ', key, value);
            if (value > limits[key]) game.possible = false;
            game.power = game.power * value;
        }
        if (DEBUG) console.log('=========================================');
        return game;
    });

    games.forEach(({ id, possible, pulls, max, power }) => {
        console.log('id, possible, max :>> ', id, possible, max);
        part2 += power;
        if (possible) part1 += id;
        else impossible += id;
        total += id;
    });
    ...
    console.log('Part 1:', part1); //2476
    console.log('impossible :>> ', impossible);
    console.log('total :>> ', total);
    console.log('Part 2:', part2); //54911
}
