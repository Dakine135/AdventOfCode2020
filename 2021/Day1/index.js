...
function main(input) {
  ...
  let countIncrease = 0;
  input.forEach((entry) => {
    if (previous != null && entry > previous) countIncrease++;
    previous = entry;
  });

  //part2
  let previousSum = null;
  let countIncreaseSum = 0;
  for (let i = 2; i < input.length; i++) {
    let sum = input[i] + input[i - 1] + input[i - 2];
    if (previousSum != null && sum > previousSum) countIncreaseSum++;
    previousSum = sum;
  }
  ...
  console.log("Sum of counts Part 1:", countIncrease);
  console.log("Sum of counts Part 2:", countIncreaseSum); //NOT 479
}
